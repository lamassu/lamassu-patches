...
usage() {
  echo "Usage: $0 \"Operator Name\"" >&2
}

operator_name="${1:-}"
if [[ $# -ne 1 || -z "$operator_name" ]]; then
  usage
  exit 2
fi

if [[ ${#operator_name} -gt 120 || "$operator_name" =~ [[:cntrl:]] ]]; then
  echo "ERROR: Operator name must be one line of 1-120 printable characters." >&2
  exit 2
fi
...
operator_file="$(root_path /etc/lamassu/monthly-volume-operator-name)"
...
operator_payload="$tmp_dir/monthly-volume-operator-name"
...
SUPPORT_KEY="${SUPPORT_KEY:-/etc/lamassu/keys/lamassu-log-server.key}"
OPERATOR_FILE="${OPERATOR_FILE:-/etc/lamassu/monthly-volume-operator-name}"
...
if [[ -n "$OPERATOR_NAME_OVERRIDE" ]]; then
  operator_name="$OPERATOR_NAME_OVERRIDE"
elif [[ -r "$OPERATOR_FILE" ]]; then
  IFS= read -r operator_name < "$OPERATOR_FILE" || true
else
  echo "ERROR: Operator name file not readable: $OPERATOR_FILE" >&2
  exit 1
fi
...
if [[ -z "$operator_slug" ]]; then
  echo "ERROR: Operator name does not produce a safe filename." >&2
  exit 1
fi
...
archive_name="${operator_slug}-${hostname_value}-${report_month}-monthly-volume.tar.gz"
...
install_payload "$operator_payload" "$operator_file" 600
