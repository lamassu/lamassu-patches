...
if [[ "$UBUNTU_VERSION" == "20.04" ]]; then
  ...
  echo
  echo -e "\033[1mUpdating lamassu-server to Grand Gilgamesh v8.1.2...\033[0m"
  echo
...
#        curl -o $(npm root -g)/lamassu-server/bin/lamassu-eth-sweep-to-new-wallet https://raw.githubusercontent.com/lamassu/lamassu-server/release-8.0/bin/lamassu-eth-sweep-to-new-wallet >> ${LOG_FILE} 2>&1
  ...
  decho "updating lamassu-server to Grand Gilgamesh v8.1.2..."
  sourceHash=$'cb12b5ad6554056227d55bf10dfa4683d1b5faeccd8025f8ab727f289b6f820b'
  curl -sSLo /tmp/lamassu-server.tar.gz https://github.com/lamassu/lamassu-server/releases/download/v8.1.2/lamassu-server-v8.1.2.tar.gz >> $LOG_FILE 2>&1
  ...
  rm /usr/bin/hkdf
  rm /usr/bin/bip39
  cp -s /usr/lib/node_modules/lamassu-server/bin/lamassu-* /usr/bin/
  cp -s /usr/lib/node_modules/lamassu-server/bin/hkdf /usr/bin/
  cp -s /usr/lib/node_modules/lamassu-server/bin/bip39 /usr/bin/
  chmod +x /usr/lib/node_modules/lamassu-server/bin/
  set -e
