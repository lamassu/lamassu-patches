...
curl -#o /tmp/bitcoincash.tar.gz https://download.bitcoinabc.org/0.20.9/linux/bitcoin-abc-0.20.9-x86_64-linux-gnu.tar.gz >> ${LOG_FILE} 2>&1
...
cp /tmp/bitcoin-abc-0.20.9/bin/bitcoind /usr/local/bin/bitcoincashd >> ${LOG_FILE} 2>&1
cp /tmp/bitcoin-abc-0.20.9/bin/bitcoin-cli /usr/local/bin/bitcoincash-cli >> ${LOG_FILE} 2>&1
rm -r /tmp/bitcoin-abc-0.20.9 >> ${LOG_FILE} 2>&1
