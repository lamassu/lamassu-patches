...
echo
echo "Downloading..."
curl -#o /tmp/ethereum.tar.gz https://gethstore.blob.core.windows.net/builds/geth-linux-amd64-1.9.5-a1c09b93.tar.gz >> ${LOG_FILE} 2>&1
tar -xzf /tmp/ethereum.tar.gz -C /tmp/ >> ${LOG_FILE} 2>&1
echo
echo "Updating..."
mv /usr/local/bin/geth /usr/local/bin/geth-old >> ${LOG_FILE} 2>&1
cp /tmp/geth-linux-amd64-1.9.5-a1c09b93/geth /usr/local/bin/geth >> ${LOG_FILE} 2>&1
rm -r /tmp/geth-linux-amd64-1.9.5-a1c09b93/ >> ${LOG_FILE} 2>&1
