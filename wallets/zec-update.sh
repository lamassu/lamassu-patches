...
echo "Downloading Zcash v4.4.0..."
curl -#Lo /tmp/zcash.tar.gz https://z.cash/downloads/zcash-4.4.0-linux64-debian-stretch.tar.gz >> ${LOG_FILE} 2>&1
tar -xzf /tmp/zcash.tar.gz -C /tmp/ >> ${LOG_FILE} 2>&1
echo

echo "Updating wallet..."
cp /tmp/zcash-4.4.0/bin/* /usr/local/bin/ >> ${LOG_FILE} 2>&1
rm -r /tmp/zcash-4.4.0 >> ${LOG_FILE} 2>&1
